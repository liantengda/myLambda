...
public class LambdaTest {
    ...
    public static void main(String[] args) {
        Person black = new Person("小黑", 11L,"hei");
        Person red = new Person("小红",12L,"hong");
        Person yellow = new Person("小黄", 13L,"huang");
        Person green = new Person("小绿", 15L,"lv");
        Person red2 = new Person("小红",12L,"hong");
        Person green2 = new Person("小绿", 15L,"lv");
        ...
        Map<String, Long> collect16 = personArrayList.stream().collect(Collectors.groupingBy(p -> p.getName(), Collectors.summingLong(Person::getPersonId)));
        //将某属性集合到一个集合中
        ...
        //双冒号的用法
        personArrayList.forEach(Person::getName);
        ...
        List<Person> 黄 = personArrayList.stream().filter(item -> item.getName().contains("黄")).collect(Collectors.toList());
        ...
        Map<Long, String> collect10 = collect11.stream().collect(Collectors.toMap(Person::getPersonId, Person::getName));
        System.out.println(collect10);
        //将list转成Map
        ArrayList<Person> collect13 = personArrayList.stream().collect(Collectors.collectingAndThen(Collectors.toCollection(
                () -> new TreeSet<>(Comparator.comparing(Person::getPersonId))
        ), ArrayList::new));
        Map<Long, Person> collect12 = collect13.stream().collect(Collectors.toMap(Person::getPersonId, Person -> Person));
        System.out.println(collect12);
        //按照某个字段，将List转为Map
        Map<Long, List<Person>> collect14 = personArrayList.stream().collect(Collectors.groupingBy(Person::getPersonId));
        System.out.println(collect14);
        Map<String, List<Person>> collect24 = personArrayList.stream().collect(Collectors.groupingBy(Person::getName));
        System.out.println(collect24);
        Map<String, Long> collect25 = personArrayList.stream().collect(Collectors.toMap(Person::getName, Person::getPersonId));
